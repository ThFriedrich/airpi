...
from ap_architectures.utils import tf_binary_mask, true_dict, pred_dict
...
def loss(prms):
    ...
    loss_fcns_r = [px_p, px_a, px_pw, px_int, px_obj]
    loss_fcns_k = [px_p, px_a, px_pw, px_int, px_cst_int, df_ratio]
    lo_r = []
...
def metric_fcns(prms, prms_net=None, model=None):
    ...
    b_normalize = tf.constant(False, dtype=tf.bool)
    metrics = []
    # btw_filter = tf.cast(tf_butterworth_filter2D([128, 128], 0.5, 6, 'ci'), tf.float32)
    ...
    metric_fcns_r = [px_pw, px_a, px_int, px_obj]
    metric_fcns_k = [
        ecn,
        px_p,
        px_a,
...
def ssi_p(y_true, y_pred, b_normalize=tf.constant(False, dtype=tf.bool)):
    ls = fcn_ssim(y_true["sin"], y_pred["sin"])
    ls += fcn_ssim(y_true["cos"], y_pred["cos"])
...
@tf.function
def px_dn_amp(y_true, y_pred, b_normalize=tf.constant(False, dtype=tf.bool)):
...
def px_int(y_true, y_pred, b_normalize=tf.constant(True, dtype=tf.bool)):
    ls = fcn_pix_loss(y_true["int"], y_pred["int"], True)
    return ls
...
def px_obj(y_true, y_pred, b_normalize=tf.constant(True, dtype=tf.bool)):
    ls = fcn_pix_loss_weighted(y_true["obj"], y_pred["obj"], y_true["weight"], True)
    return ls
...
def xy2dict(
    ...
    ypr["obj"] = tf.math.angle(ypr["wv"]/ytr["probe"])
...
def plot_yt_yp(ytr, ypr, ytk, ypk):
    ...
    plt.imshow(ytk["amp"][n, ...])
    plt.colorbar()
    plt.subplot(5, 3, 2).set_axis_off()
    plt.imshow(ypk["amp"][n, ...])
    plt.colorbar()
    plt.subplot(5, 3, 3).set_axis_off()
    plt.imshow((ytk["amp"][n, ...] - ypk["amp"][n, ...]))
    plt.colorbar()
