...
from numpy import sqrt, shape, cast
from math import pi
...
tf_pi = tf.constant(pi, dtype=tf.float32)
...
def tf_complex_interpolation(y_cpx=None, y_amp=None, y_phase=None, out_size=[64,64], output='complex'):
    '''
    ...
    b_even = out_size[0] % 2 == 0
    nx = out_size[1]
    ny = out_size[0]
    if b_even:
        y_cpx = tf_pad2d(y_cpx,[0,1])
        nx += 1
        ny += 1
    if len(y_cpx.shape) == 3:
        y_cpx = tf.expand_dims(y_cpx, axis=-1)
    else:
        y_cpx = y_cpx[tf.newaxis,...,tf.newaxis]
    # y_cpx = y_cpx[..., tf.newaxis]
    re = tf.image.resize(tf.math.real(y_cpx), [ny,nx],antialias=True)
    im = tf.image.resize(tf.math.imag(y_cpx), [ny,nx],antialias=True)
    cpx = tf.complex(re, im)
    if b_even:
        cpx = cpx[:,:-1,:-1,:]
    cpx = tf.squeeze(cpx)
    if output=='complex':
...
def fft2d(x):
    """2D FFT using numpy"""
    nx = cast[x.dtype](shape(x)[1])
    return fftshift(fft2(ifftshift(x)))/nx
...
def tf_fft2d(x):
    ...
    return tf.signal.fftshift(tf.signal.fft2d(tf.signal.ifftshift(x)))/nx
...
def tf_ifft2d(x):
    ...
    nx = tf.cast(tf.shape(x)[1],x.dtype)
    return tf.signal.fftshift(tf.signal.ifft2d(tf.signal.ifftshift(x)))*nx
...
def tf_fft2d_A_p(A_p: tf.Tensor, complex_out:tf.bool=False) -> tf.Tensor:
    ...
    A_p[:, :, 1] = phase
    """
    nx = tf.cast(tf.shape(A_p)[1],tf.complex64)
...
def tf_probe_k(
    ...
    if output_type == "real":
        ...
        return tf_cast_complex(mask_k_i, mask_k_p)
...
def tf_probe_function(
    ...
    amp, phase = tf_probe_k(E0, aperture, gmax, nx, aberration)

    if domain == "r":
        amp, phase = tf_switch_space(tf_cast_complex(amp, phase))
        if refine:
            amp_o = amp[nx//3:nx//3*2, nx//3:nx//3*2]
            phase_o = phase[nx//3:nx//3*2, nx//3:nx//3*2]
            if type == "real":
                return amp_o
            elif type == "complex":
                return tf_cast_complex(amp_o, phase_o)
    else:
        if refine:
            amp, phase = tf_complex_interpolation(y_amp=amp, y_phase=phase, out_size=[nx//3, nx//3], output="real")

    if type == "real":
        return amp
    elif type == "complex":
        return tf_cast_complex(amp, phase)
...
def pred_dict(y_tensor):
    ...
    r["int"] = tf.math.pow(r["amp"],2)
    r["sin"], r["cos"] = tf_sin_cos_decomposition(r["phase"])

    wv_s = tf_cast_complex(r["amp"], r['phase'])
    k["amp"], k["phase"] = tf_switch_space(wv_s, space="r")
    k["wv"] = tf_cast_complex(k["amp"], k["phase"])
    k["amp_sc"] = tf.math.pow(k["amp"],0.2)
    k["sin"], k["cos"] = tf_sin_cos_decomposition(k["phase"])
    k["int"] = tf.math.abs(tf.math.pow(k["wv"],2))
...
def true_dict(y_tensor):
    ...
    k['bf_frac'] = tf.math.sqrt(tf.reduce_sum(k["msk"],axis=[1,2], keepdims=True)/tf_pi)/32.0 
    ...
    k["amp_sc"] = k["amp"] ** 0.2
    k['msk'] = tf.cast(msk_r,tf.float32)
    return r, k
